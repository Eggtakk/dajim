...
import { suggestGoalReset, suggestNextGoal } from "./suggestGoalReset";
import type { CategoryTrend } from "./suggestGoalReset";
import {
  buildIndexSeries,
...
function getOtherCategoryTrends(goal: GoalSettings): CategoryTrend[] {
  return CATEGORIES.filter((c) => c.id !== goal.categoryId).map((c) => ({
    categoryId: c.id,
    label: c.label,
    changePct: predictCategoryTrend(getWeeklySpendHistory(c.id)).changePct,
  }));
}
...
export function getResultData(
  ...
  const otherCategoryTrends = getOtherCategoryTrends(goal);

  if (outcome === "win") {
    const actualPct = goal.percent + 4;
    return {
      outcome,
      ...
      suggestions: suggestNextGoal(goal, otherCategoryTrends),
    };
  }

  const actualPct = Math.max(1, Math.round(goal.percent * 0.3));
  const gapPct = goal.percent - actualPct;
  ...
  return {
    ...
    suggestions: suggestGoalReset(goal, gapPct, otherCategoryTrends),
  };
}
